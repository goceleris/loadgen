// Package checkpoint provides incremental benchmark progress tracking.
// It persists benchmark results to disk so that interrupted benchmark
// suites can resume without re-running completed configurations.
package checkpoint
...
// NewServerResult converts a loadgen.Result to a ServerResult.
func NewServerResult(r *loadgen.Result, server, benchmark, method, path string) ServerResult {
	sr := ServerResult{
		Server:         server,
		Benchmark:      benchmark,
		Method:         method,
